...
public static class LobbyHarmonyPatches
{
	...
	private static bool _autoRandomizePending = false;

	private static int _autoRandomizeLastPlayerCount = -1;

	private static float _autoRandomizeCooldown = 0f;

	private static float DEFAULT_LOBBY_TIMER_DURATION
	{
		get
		{
			if (!(ServerData.Config.LobbyTimerDuration > 0f))
			{
				return 600f;
			}
			return ServerData.Config.LobbyTimerDuration;
		}
	}

	private static int VANILLA_MAX_PLAYERS
	{
		get
		{
			if (ServerData.Config.ModDetectMaxPlayers <= 0)
			{
				return 15;
			}
			return ServerData.Config.ModDetectMaxPlayers;
		}
	}

	private static int VANILLA_MAX_IMPOSTORS
	{
		get
		{
			if (ServerData.Config.ModDetectMaxImpostors <= 0)
			{
				return 3;
			}
			return ServerData.Config.ModDetectMaxImpostors;
		}
	}

	private static float VANILLA_MAX_SPEED
	{
		get
		{
			if (!(ServerData.Config.ModDetectMaxSpeed > 0f))
			{
				return 3f;
			}
			return ServerData.Config.ModDetectMaxSpeed;
		}
	}

	private static float VANILLA_MIN_KILL_CD
	{
		get
		{
			if (!(ServerData.Config.ModDetectMinKillCd > 0f))
			{
				return 10f;
			}
			return ServerData.Config.ModDetectMinKillCd;
		}
	}
	...
	public static void InitializeConfig(ConfigFile config)
	{
		if (config == null)
		{
			Debug.LogError(Object.op_Implicit("[LobbyPatches] ConfigFile is null!"));
			return;
		}
		Config = config;
		...
		if (Enum.TryParse<Lang>(cfgLanguage.Value, ignoreCase: true, out var result))
		{
			CurrentLang = result;
		}
	...
	private static (bool IsModded, string Reason) DetectIfModded(GameListing listing)
	{
		...
		{
			if (listing.HostName.IndexOf(value, StringComparison.OrdinalIgnoreCase) >= 0)
			{
		...
		if (listing.MaxPlayers > VANILLA_MAX_PLAYERS)
		{
		...
		if (!IsHideAndSeekMode(options.GameMode))
		{
			...
			try
			{
				float @float = options.GetFloat((FloatOptionNames)2);
				float float2 = options.GetFloat((FloatOptionNames)1);
				if (@float > VANILLA_MAX_SPEED || float2 < VANILLA_MIN_KILL_CD)
				{
					return (IsModded: true, Reason: UI("InvalidRules"));
				}
			}
	...
	private static string FormatGameSettings(IGameOptions options)
	{
		...
		else
		{
			...
			try
			{
				float @float = options.GetFloat((FloatOptionNames)1);
				_sbSettings.Append(UI("KillCD")).Append(": ").Append(Colorize($"{@float:F0}s", "#FFFFFF"))
					.Append(" | ");
				float float2 = options.GetFloat((FloatOptionNames)2);
				_sbSettings.Append(UI("Speed")).Append(": ").Append(Colorize($"{float2:F1}x", "#FFFFFF"));
			}
			catch
			{
			}
	...
	private static string FormatRolesInfo(IGameOptions options)
	{
		...
		{
			return "\n<size=30%>" + Colorize("• " + UI("Roles"), "#a7f") + "</size>";
		}
		return "";
	}
	...
	public static void OnLobbyStart()
	{
		...
		try
		{
			...
			ChatController val = ((instance2 != null) ? instance2.Chat : null);
		...
		_autoRandomizePending = JoinedAsHost;
		_autoRandomizeLastPlayerCount = -1;
	...
	public static void OnLobbyUpdate(LobbyBehaviour __instance)
	{
		...
		if (!JoinedAsHost || !(Time.time >= _autoRandomizeCooldown))
		{
			return;
		}
		int num = PlayerControl.AllPlayerControls?.Count ?? 0;
		if (num >= 4 && num != _autoRandomizeLastPlayerCount)
		{
			try
			{
				PlayerPickMenu.AssignRolesForNewPlayersSilent();
			}
			catch
			{
			}
			_autoRandomizeLastPlayerCount = num;
			_autoRandomizeCooldown = Time.time + 1.5f;
		}
	}
	...
	public static void OnPlayerJoinedPatch(ClientData data)
	{
		...
		try
		{
			if ((Object)(object)AmongUsClient.Instance != (Object)null && ((InnerNetClient)AmongUsClient.Instance).AmHost && !string.IsNullOrEmpty(data.FriendCode) && data.Id != ((InnerNetClient)AmongUsClient.Instance).HostId && CrewBanListService.IsBanned(data.FriendCode))
			{
				int id = data.Id;
				string text = data.PlayerName ?? "player";
				((InnerNetClient)AmongUsClient.Instance).KickPlayer(id, true);
				try
				{
					NotifyUtils.Warning("Auto-banned " + text + " (on banlist)");
				}
				catch
				{
				}
				DebugLog($"Auto-ban on join: {text} ({data.FriendCode})");
				return;
			}
		}
		catch (Exception value)
		{
			Debug.LogError(Object.op_Implicit($"[CrewBanList] auto-kick error: {value}"));
		}
		if ((Object)(object)data.Character != (Object)null)
		{
			PlayerCount++;
			try
			{
			...
			try
			{
				GameCheats.ScheduleAutoKickCheck();
			}
			catch
			{
			}
	...
	public static void OnPlayerLeftPatch(ClientData data, DisconnectReasons reason)
	{
		...
		try
		{
			if ((Object)(object)AmongUsClient.Instance != (Object)null && ((InnerNetClient)AmongUsClient.Instance).AmHost && !string.IsNullOrEmpty(data.FriendCode) && data.Id != ((InnerNetClient)AmongUsClient.Instance).HostId)
			{
				RecentlyLeftService.Track(data.FriendCode, data.PlayerName ?? "");
			}
		}
		catch
		{
		...
		try
		{
			if ((Object)(object)AmongUsClient.Instance != (Object)null && ((InnerNetClient)AmongUsClient.Instance).AmHost && (Object)(object)ShipStatus.Instance == (Object)null && ImpostorForcer.PurgeStaleAssignments() > 0)
			{
				try
				{
					PlayerPickMenu.AssignRolesForNewPlayersSilent();
		...
		try
		{
			byte playerId = (((Object)(object)data.Character != (Object)null) ? data.Character.PlayerId : byte.MaxValue);
			GameCheats.OnClientLeft(data.Id, playerId);
		}
		catch
		{
		}
	...
	private static void DebugLog(string message, LogLevel level = LogLevel.Info)
	{
		...
		if (obj != null && obj.Value)
		{
			...
			case LogLevel.Warning:
				Debug.LogWarning(Object.op_Implicit(text));
				break;
			case LogLevel.Error:

...
public static class NotifyUtils
{
	...
	private static readonly Color MOD_NOTIFY_COLOR = GuiStyles.Theme.Visor;
	...
	{
		Show(ToastType.Success, "<b><color=#00FF88>[OK]</color></b> " + msg);
	}

	public static void Error(string msg)
	{
		Show(ToastType.Error, "<b><color=#FF4444>[X]</color></b> " + msg);
	}

	public static void Warning(string msg)
	{
		Show(ToastType.Warning, "<b><color=#FFAA00>[!]</color></b> " + msg);
	}

	public static void Info(string msg)
	{
		Show(ToastType.Info, "<b><color=#44BBFF>[i]</color></b> " + msg);
	}

	public static void Sabotage(string msg)
	{
		Show(ToastType.Error, "<b><color=#FF5555>[SAB]</color></b> " + msg);
	}

	public static void Door(string msg)
	{
		Show(ToastType.Warning, "<b><color=#FF8800>[DOOR]</color></b> " + msg);
	}

	public static void Repair(string msg)
	{
		Show(ToastType.Success, "<b><color=#44FF88>[FIX]</color></b> " + msg);
	}

	public static void Show(string message)
	{
		Show(ToastType.Info, message);
	}

	private static void Show(ToastType type, string message)
	{
		...
		for (int i = 0; i < _activeCount; i++)
		{
			...
			try
			{
				...
				{
					string text = message + $" <size=80%><color=#949EAD>(x{_active[i].Count})</color></size>";
	...
	{
		if (_pending.Count == 0 || !DestroyableSingleton<HudManager>.InstanceExists)
		{
			return;
		}
	...
	private static bool TryDeliver(string message, float now)
	{
		...
		{
			Debug.LogError(Object.op_Implicit("[NotifyUtils] Delivery failed: " + ex.Message));
			return false;
		}
	}
	...
	private static void DeliverCustom(NotificationPopper popper, string message, float now)
	{
		...
		newMsg.SetUp(message, popper.settingsChangeSprite, MOD_NOTIFY_COLOR, Action.op_Implicit((Action)delegate
		{
			RemoveActive(newMsg);
		...
		try
		{
			if (DestroyableSingleton<SoundManager>.InstanceExists)
			{

...
public class ReplayPlayer : MonoBehaviour
{
	...
	private Texture2D _cachedDummyTex;
	...
	public ReplayPlayer(IntPtr ptr)
		: base(ptr)
	{
	}
	...
	public void SpawnPuppet(ReplayPlayerInfo info)
	{
		...
		try
		{
			...
			if (!flag2)
			{
				...
				{
					val3.color = Color32.op_Implicit(((Il2CppArrayBase<Color32>)(object)Palette.PlayerColors)[info.ColorId]);
				}
				else
				{
					val3.color = Color32.op_Implicit(info.RealColor);
				}
			}
			...
			((TMP_Text)obj4).outlineColor = Color32.op_Implicit(Color.black);
	...
	private Sprite GetDummySprite()
	{
		...
		if ((Object)(object)_cachedDummyTex != (Object)null)
		{
			return Sprite.Create(_cachedDummyTex, new Rect(0f, 0f, 64f, 64f), new Vector2(0.5f, 0.5f), 32f);
		}
		int num = 64;
		int num2 = 64;
	...
	private void ApplyFrameState(float time)
	{
		...
		List<ReplayFrame> frames = currentData.Frames;
		...
		foreach (PlayerState state in replayFrame.States)
		{
			...
			if (replayFrame2 != null)
			{
				int num4 = replayFrame2.States.FindIndex((PlayerState s) => s.PlayerId == state.PlayerId);
				if (num4 >= 0)
				{
					pos = Vector2.Lerp(state.Position, replayFrame2.States[num4].Position, num2);
				}
			}
	...
	public void Cleanup()
	{
		...
		if ((Object)(object)_cachedDummyTex != (Object)null)
		{
			try
			{
				Object.Destroy((Object)(object)_cachedDummyTex);
			}
			catch
			{
			}
			_cachedDummyTex = null;
		}
	}

	...
	internal static object GetUIState()
	{
		...
		try
		{
			...
			foreach (KeyValuePair<byte, RoleTypes> preGameRoleAssignment in ImpostorForcer.PreGameRoleAssignments)
			{
				...
				preAssignBuffer[key] = ((object)(RoleTypes)(ref val)).ToString();
			}
			...
			return new
			{
				...
				hostRoleOverrideRole = ((object)(RoleTypes)(ref val)).ToString(),
		...
		try
		{
			...
			try
			{
				...
				foreach (KeyValuePair<byte, RoleTypes> item in preGameRoleAssignments)
				{
					...
					int num7 = num * 31;
					val3 = item.Value;
					num = num7 + ((object)(RoleTypes)(ref val3)).GetHashCode();
				}
			}
			catch
			{
			...
			try
			{
				...
				num = num8 + ((object)(RoleTypes)(ref val3)).GetHashCode();
			}
			catch
			{
			}
